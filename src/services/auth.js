...
export const login = ({ email, password }) => new Promise(async (resolve, reject) => {
    try {
        ...
        const token = isCheckedPassword ? jwt.sign({id: response.id, email: response.email, role_code: response.role_code}, process.env.JWT_SECRET, { expiresIn: '5d'}) : null

        resolve({
            err: token ? 0 : 1,
            mes: token ? 'Login is successfully' : response ? 'Password is wrong' : 'Email has already been registered',
            'access_token': token ? `Bearer ${token}` : null
        })
    } catch (error) {
        reject(error)
